...
\end{exercisebox}
% Note that this factor exactly matches the prefactor of $F_2$ in \cref{eq:dsigmadxdy}.
% All remaining factors to obtain the full expression for the (double differential) cross sections are given by considering the actual phase space (which we neglect here) and other global prefactors, e.g.\ the flux.
% At LO it is the overall momentum conservation delta function in the phase space which survives and eventually the LO coefficient function for $F_2$ is given by \cref{eq:LOC}, i.e.\ only the factors $\qty(g_{V}^{\gamma q})^2 = e_q^2$ in \cref{eq:LOsigma} are accounted for there (and not elsewhere).
