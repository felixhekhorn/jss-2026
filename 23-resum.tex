...
\end{equation}
While for the special case $\mu_0 = \mu_R$ we need to be consistent of course and we thus find that $\alpha_s^\text{LO}(\mu_R^2)$ is $\alpha_s(\mu_0^2)$ at the LO truncation, we note that they differ by higher order terms.
...
Starting from NLO accuracy the RGE,
\begin{equation}
    \mu_R^2 \dv{\alpha_s(\mu_R^2)}{\mu_R^2} = - \beta_0 \qty(\alpha_s(\mu_R^2))^2 - \beta_1 \qty(\alpha_s(\mu_R^2))^3\,, \label{eq:NLObeta}
\end{equation}
can no longer be solved exactly with elementary function, which is a problem.
...
We refer to the literature on how to implement a specific solution in practice and remain here just with the fact that it is a complicated problem, which has no unique solution and for which approximations are mandatory.
\begin{conclusionbox}[label={RGE}]{Solution of RGE}
    \begin{itemize}
        \item The LO RGE of the strong coupling, \cref{eq:LObeta}, can be solved exactly, \cref{eq:LOalphas}
        \item The NLO RGE of the strong coupling, \cref{eq:NLObeta}, can not be solved exactly and one needs to choose a solution method
    \end{itemize}
\end{conclusionbox}
