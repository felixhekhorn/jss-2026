...
In practice all modern PDFs are determined in the $\overline{\mathrm{MS}}$ scheme and thus not of any practical relevance, but it can provide some practical advice based on theoretical considerations.

Before making explicit use of the DIS scheme, we review the interplay with the previous content.
\begin{itemize}
    \item First, we need all possible DIS combinations (NC vs.\ CC) to define all quark combinations, see \cref{sec:diverse}.
        Second, this only defines quarks, but not the gluon, which is explicitly defined to be not contributing.
...
The difference between \cref{eq:CpQCD} (implicitly in $\overline{\mathrm{MS}}$ scheme) and \cref{eq:CDIS} is, by construction, a perturbative object and thus the scheme transformation between these two schemes.
